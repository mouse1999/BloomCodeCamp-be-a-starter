...
//        when(userRepository.findById(userId)).thenReturn(Optional.of(user));
        ...
        AssignmentModel actualAssignment = assignmentService.createAssignment(assignmentNumber, user);
        ...
        assertEquals(AssignmentStatusEnum.PENDING_SUBMISSION.getStatus(), actualAssignment.getStatus(),
    ...
    void createAssignment_invalidAssignmentNumber_throwsInvalidAssignmentException() {
        ...
        when(assignmentRepository.existsByAssignmentNumberAndUserId(invalidAssignmentNumber, userId))
                .thenReturn(false);

        assertThrows(InvalidAssignmentNumberException.class, () -> {
            assignmentService.createAssignment(invalidAssignmentNumber, user);
        });
...
//
    ...
    void createAssignment_duplicateAssignmentForSameUser_throwsAssignmentAlreadySubmittedException() {
        ...
        assertThrows(AssignmentAlreadySubmittedException.class, () -> {
            assignmentService.createAssignment(assignmentNumber, user);
        });
    }
...
//        assertEquals(branchName, result.getBranch(),"branch name must match");
//        assertEquals(githubUrl, result.getGithubUrl(),"githuburl must match");
...
//        assertNotNull(result.getCodeReviewer(),"reviewer must not be null");
